...
} from '@/features/work-attendance/hooks/useWorkSchedule';
import { typography } from '@/theme/typography';
import { lightTheme, colors, semantic } from '@/theme/colors';
import { spacing } from '@/theme/spacing';
import { normalize } from '@/theme/normalize';

const DAYS_OF_WEEK = [0, 1, 2, 3, 4, 5, 6]; // Sun=0 through Sat=6

// ─── Teacher Work Schedule Editor ────────────────────────────────────────────

export default function TeacherWorkScheduleScreen() {
  const { t } = useTranslation();
  const router = useRouter();
  const { id: teacherId } = useLocalSearchParams<{ id: string }>();
  const { schoolId } = useAuth();

  const { data: schedules = [], isLoading, error, refetch } = useWorkSchedules(teacherId);
  const upsertMutation = useUpsertWorkSchedule();
  ...
  const handleSave = () => {
    if (editingDay === null || !teacherId || !schoolId) return;
  ...
  const handleDelete = (scheduleId: string, dayName: string) => {
    Alert.alert(
      t('admin.workSchedule.removeTitle'),
      t('admin.workSchedule.removeMessage', { day: dayName }),
      [
  ...
  return (
    <Screen scroll>
      <View style={styles.container}>
        ...
        <Text style={styles.title}>{t('admin.workSchedule.title')}</Text>
        <Text style={styles.description}>{t('admin.workSchedule.description')}</Text>
              ...
              <View style={styles.dayRow}>
                <View style={styles.dayInfo}>
                  ...
                  {schedule && !isEditing ? (
                    ...
                    <Text style={styles.offLabel}>{t('admin.workSchedule.dayOff')}</Text>
                  ) : null}
                </View>
                {!isEditing && (
                  <View style={styles.dayActions}>
                    ...
                    <Button
                      title={schedule ? t('common.edit') : t('admin.workSchedule.addHours')}
