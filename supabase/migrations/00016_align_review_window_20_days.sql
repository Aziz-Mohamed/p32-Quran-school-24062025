-- Align the recent/old review boundary with the traditional Quran memorization
-- methodology (Manhaj al-Hifz). The Muraja'ah Qareebah (near revision) window
-- is ~20 days in the traditional approach. This replaces the previous 7-day boundary.
...
AS $$
  ...
  -- Near revision (المراجعة القريبة): memorized within last 20 days, due for review
  SELECT
  ...
  -- Far revision (المراجعة البعيدة): memorized more than 20 days ago, due for review
  SELECT
    mp.id AS progress_id,
    mp.surah_number,
    mp.from_ayah,
