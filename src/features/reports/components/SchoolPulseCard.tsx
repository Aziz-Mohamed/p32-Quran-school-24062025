...
const STATUS_CONFIG: Record<HealthStatus, { color: string; bg: string; icon: string }> = {
  green: { color: semantic.success, bg: semanticSurface.success, icon: 'shield-checkmark' },
...
interface SchoolPulseCardProps {
  status: HealthStatus;
  message: string;
  isLoading?: boolean;
}

export function SchoolPulseCard({ status, message, isLoading }: SchoolPulseCardProps) {
