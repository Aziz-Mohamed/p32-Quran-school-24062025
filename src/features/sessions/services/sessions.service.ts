...
class SessionsService {
  ...
  async getSessions(filters: SessionFilters) {
    let query = supabase
      .from('sessions')
      .select(
        '*, profiles!sessions_teacher_id_fkey(full_name, avatar_url), profiles!sessions_student_id_fkey(full_name, avatar_url), lessons(title)',
      );
  ...
  async getSessionById(id: string) {
    return supabase
      .from('sessions')
      .select(
        '*, profiles!sessions_teacher_id_fkey(full_name, avatar_url), profiles!sessions_student_id_fkey(full_name, avatar_url), lessons(title), homework(*)',
      )
      .eq('id', id)
      .single();
  }
